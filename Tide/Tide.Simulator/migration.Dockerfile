...
CMD ["bash", "-c", "for i in 1 2 3 4 5 6; do ( echo 'trying migrations' && sqlcmd -b -S ${SA_HOST:-127.0.0.1} -U sa -P ${SA_PASSWORD:-P@ssw0rd} -i migration.sql ) && sw=true && break || sleep 5; done && [[ $sw == true ]] && ( echo 'running service' && nc -l -p 80 -w 5 ) || ( echo 'error' && exit 1 ) "]
