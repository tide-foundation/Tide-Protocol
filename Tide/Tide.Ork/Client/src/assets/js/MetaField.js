...
export default class MetaField {
  ...
  encrypt(key, tag = null) {
    ...
    const tagCipher = tag || (this._previous.length && Cipher.tag(this._previous)) || new Num64(0);
  ...
  static fromModel(data, encrypted) {
    ...
    return Object.keys(data).map((field) => MetaField.fromText(field, data[field], encrypted));
  }
