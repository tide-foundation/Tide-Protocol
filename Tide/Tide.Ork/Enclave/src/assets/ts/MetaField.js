...
export default class MetaField {
  ...
  set value(val) {
    if (this._isEncrypted) throw new Error("Value cannot be modified if it is encrypted");
  ...
  get isValid() {
    ...
    return true; //new Validator({ val: this._value }, { val: this.valRules }).passes();
  }

  /**@type {MetaOption[]}*/
  get options() {
    return this._class.options();
  }
  get isInput() {
    return this._class.fieldType === "input";
  }
  get isSelect() {
    return this._class.fieldType === "select";
  }
  //
  /**
   * @private
  ...
  encrypt(key) {
    ...
    const tagCipher = (!this.tag.isZero && this.tag) || (this._previous.length && Cipher.tag(this._previous)) || new Num64(0);
