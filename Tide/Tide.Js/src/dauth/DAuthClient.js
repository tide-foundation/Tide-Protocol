...
export default class DAuthClient {
    ...
    async GetShare(pass) {
        var res = await superagent.get(`${this.url}/dauth/${encodeBase64Url(this.userBuffer)}/share/${encodeBase64Url(pass.toArray())}`);
    ...
    async signIn(ticks, sign) {
        ...
        var res = await superagent.get(`${this.url}/dauth/${usr}/signin/${tck}/${sgn}`);
...
function encodeBase64Url(input) {
    const text = typeof input === "string" ? input : Buffer.from(input).toString('base64');
